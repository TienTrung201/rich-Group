...
import { headerBanner, Listnewspapers } from "@/redux/selector";
...
function Header() {
  ...
  const banners = useSelector(headerBanner);
  return (
    <header className=" ">
      ...
      {/* End Nav */}
      <div className={cx("headerBanner")}>
        <ul className={cx("BannerNav", "grid", "wide")}>
          {banners.map((bannerIcon, index) => {
            return (
              <li key={index} className={cx("BannerNav__item")}>
                <Link to="#" className={cx("bannerLink")}>
                  <img
                    src={bannerIcon.src}
                    alt="icon"
                    className={cx("iconBannerNav")}
                  />
                  {bannerIcon.name}
                </Link>
              </li>
