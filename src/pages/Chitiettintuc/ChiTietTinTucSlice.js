  ...
  initialState: {
    ...
    newspapers: [
      ...
      },
      {
        id: 2,
        title:
          "Hà Nội phát sinh nhiều điểm ùn tắc mới Hồ Tùng Mậu - Lê Đức Thọ...",
        content:
      ...
      },
      {
        id: 6,
        title:
          "Nhân sự Samsung Engineering tham gia hội đồng quản trị DNP Water",
        content:
      ...
      },
      {
        id: 8,
        title:
          "Nhiều doanh nghiệp chọn Aqua City để mở rộng hoạt động kinh doanh",
        content:
...
export const AddNews = createAsyncThunk("news/AddNews", async (newspapers) => {
  const res = await fetch("/api/fetchNews", {
    method: "POST",
    body: JSON.stringify(newspapers),
  });
  const data = await res.json();
  return data.newspaper; //sau do chay ra addCase nhớ là k có số nhiều là tê của Model
});
